...
const App = () => {
    ...
    //             setTitulos(
    //                 Object.keys(datosFormateados[0] || {}).reduce((acc, key) => {
    //                     acc[key] = key;
    //                     return acc;
    //                 }, {})
    //             );
        ...
        const fetchPokemons = async () => {
            try {
                ...
                setPokemonData(formattedData);
                    setTitulos(
                        Object.keys(formattedData[0] || {}).reduce((acc, key) => {
                            acc[key] = key;
                            return acc;
                        }, {})
                    );
    ...
    return (
        <div className="flex flex-row h-screen w-screen">
                ...
                <div className="flex flex-col gap-4 overflow-auto">
                    ...
                    <div className="flex flex-col gap-4 overflow-y-auto" >
                        <div className="flex flex-col gap-2">
                            <h2 className="dark:text-white font-semibold">Títulos (editable)</h2>

                            {Object.keys(titulos).map((key) => (
                                <div key={key} className="flex flex-col">
                                    <label className="text-xs dark:text-gray-300">{key}</label>
                                    <input
                                        type="text"
                                        value={titulos[key]}
                                        onChange={(e) =>
                                        ...
                                        className="dark:bg-slate-700 dark:text-white rounded-lg border border-gray-300 p-2"
                                    />
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="flex flex-col mt-4">
                            <h2 className="dark:text-white font-semibold">Filas (solo vista previa)</h2>

                            <textarea
                                className="dark:bg-slate-700 dark:text-white rounded-lg w-full border border-gray-300 p-3"
                                rows={10}
                                readOnly
                                value={JSON.stringify(usuariosApi.slice(0, 3), null, 2)}
                            />
                        </div>
                    </div>
                </div>
            ...
            <div className=" w-full h-full">
                <DynamicTable data={pokemonData} theme={matrixTheme} titles={titulos} />
            </div>
        </div>
    )
}
